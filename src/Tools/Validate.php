...
class Validate
{
    ...
    {
        return !empty($email) && preg_match(Common::cleanNonUnicodeSupport('/^[a-z\p{L}0-9!#$%&\'*+\/=?^`{}|~_-]+[.a-z\p{L}0-9!#$%&\'*+\/=?^`{}|~_-]*@[a-z\p{L}0-9]+[._a-z\p{L}0-9-]*\.[a-z0-9]+$/ui'), $email);
    }
    ...
    {
        return preg_match(Common::cleanNonUnicodeSupport('/^[^!<>,;?=+()@#"°{}$%:]+$/u'), stripslashes($name));
    }
    ...
    {
        return Validate::isFileName($dir);
    }
    ...
    {
        return is_null($id) OR Validate::isUnsignedId($id);
    }
    ...
    {
        if ($data == null || Validate::isNickname($data)) {
            return true;
        }
    ...
    {
        return Image::isCorrectImageFileExt($data);
    }

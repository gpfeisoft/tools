...
class Image
{
    ...
    public static function thumbnail($image, $cache_image, $size, $image_type = 'jpg')
    {
        ...
        if (!file_exists(IMAGE_CACHE_DIR . $cache_image)) {
            ...
            if (!Image::checkImageMemoryLimit($image)) {
                return false;
            }
            ...
            if ($y < $size && $x <= $max_x) {
                ...
                Image::resize($image, IMAGE_CACHE_DIR . $cache_image, $ratio_x, $size, $image_type);
            }
        }
    ...
    public static function cut($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $dst_x = 0, $dst_y = 0)
    {
        ...
        $src = array(
            'width' => $src_info[0],
            'height' => $src_info[1],
            'ressource' => Image::create($src_info[2], $src_file),
        );
        ...
        $dest['ressource'] = Image::createWhiteImage($dest['width'], $dest['height']);

        $white = imagecolorallocate($dest['ressource'], 255, 255, 255);
        imagecopyresampled($dest['ressource'], $src['ressource'], 0, 0, $dest['x'], $dest['y'], $dest['width'], $dest['height'], $dest['width'], $dest['height']);
        imagecolortransparent($dest['ressource'], $white);
        $return = Image::write($file_type, $dest['ressource'], $dst_file);
    ...
    public static function resize($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $force_type = false)
    {
        ...
        $src_image = Image::create($type, $src_file);
        ...
        if (!Image::checkImageMemoryLimit($src_file))
            return false;
        ...
        return (Image::write($file_type, $dest_image, $dst_file));
    }
    ...
    public static function validateUpload($file, $max_file_size = 0)
    {
        ...
        if (!Image::isRealImage($file['tmp_name'], $file['type']) || !Image::isCorrectImageFileExt($file['name']))
            return 'Image format not recognized, allowed formats are: .gif, .jpg, .png';
        if ($file['error'])
            return sprintf(Message::displayError('Error while uploading image; please change your server\'s settings. (Error code: %s)'), $file['error']);
    ...
    public static function imageToBase64($url = '')
    {
        if ($url == '') {
            ...
            $base64 = 'data:image/' . $imgType . ';base64,' . $fileContent;
            fclose($fp);
    ...
    public static function getTimeImageName($image_type = "image/jpeg")
    {
        if ($image_type == "image/jpeg" || $image_type == "image/pjpeg") {
            return Date::getMicroTimeSingle() . ".jpg";
        } elseif ($image_type == "image/gif") {
            return Date::getMicroTimeSingle() . ".gif";
        } elseif ($image_type == "image/png") {
            return Date::getMicroTimeSingle() . ".png";
        } else {
            return Date::getMicroTimeSingle();
        }
    }
